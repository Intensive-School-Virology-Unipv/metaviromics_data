...
if [ "$2" = "bacteria" ]; then
	...
	if [ ! -s $1/.bacteria ]; then
		...
		wget ftp.ncbi.nlm.nih.gov/genomes/refseq/archaea/assembly_summary.txt > .$2.tmp
                awk -F "\t" '$12=="Complete Genome" && $11=="latest" {print $20}' assembly_summary.txt > .$2.tmp 
		$DIR/exe/dscriptMaker .$2.tmp > ./download.sh
                ./download.sh 2> .$2.tmp
...
if [ "$2" = "viruses" ]; then
	if [ ! -s $1/.viruses ]; then
		...
		echo "Downloading now Viruses complete genomes:"
                wget ftp.ncbi.nlm.nih.gov/genomes/refseq/viral/assembly_summary.txt
                awk -F "\t" '$12=="Complete Genome" && $11=="latest" {print $20}' assembly_summary.txt > .$2.tmp
		$DIR/exe/dscriptMaker .$2.tmp > ./download.sh
                chmod 711 ./download.sh
                ./download.sh 2> .$2.tmp
...
if [ "$2" = "plasmid" ]; then
		...
		wget ftp.ncbi.nlm.nih.gov/genomes/refseq/plasmid/plasmid.1.1.genomic.fna.gz
		wget ftp.ncbi.nlm.nih.gov/genomes/refseq/plasmid/plasmid.2.1.genomic.fna.gz
		wget ftp.ncbi.nlm.nih.gov/genomes/refseq/plasmid/plasmid.3.1.genomic.fna.gz
		wget ftp.ncbi.nlm.nih.gov/genomes/refseq/plasmid/plasmid.4.1.genomic.fna.gz
...
if [ "$2" = "plastid" ]; then
        if [ ! -s $1/.plastid ]; then
                ...
                wget ftp.ncbi.nlm.nih.gov/genomes/refseq/plastid/plastid.1.1.genomic.fna.gz
                wget ftp.ncbi.nlm.nih.gov/genomes/refseq/plastid/plastid.2.1.genomic.fna.gz
                echo "Downloading done. Uncompressing files... "
...
if [ "$2" = "fungi" ]; then
	if [ ! -s $1/.fungi ]; then
		...
		wget ftp.ncbi.nlm.nih.gov/genomes/refseq/$2/assembly_summary.txt
		awk -F "\t" '$12=="Complete Genome" && $11=="latest" {print $20}' assembly_summary.txt > .$2.tmp
		$DIR/exe/dscriptMaker .$2.tmp > ./download.sh
		chmod 711 ./download.sh
		./download.sh  2> .$2.tmp
...
if [ "$2" = "human" ]; then
	  if [ ! -s $1/.human ]; then
	  	...
	  	wget ftp.ncbi.nlm.nih.gov/genomes/all/GCF/000/001/405/GCF_000001405.37_GRCh38.p11/GCF_000001405.37_GRCh38.p11_genomic.fna.gz
		echo "Downloading done. Uncompressing files... "
	  	gunzip ./*fna.gz
